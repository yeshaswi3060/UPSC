...
export default function LandingPage() {
  ...
  return <main>
    <section className="hero" id="top">
      ...
      <div className="container hero-layout">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><button className="btn btn-coral" onClick={buy} disabled={!owned && !checkoutReady}>{owned ? 'Open your library' : checkoutReady ? `Get the paper for ₹${price}` : 'Launching soon'}<ArrowUpRight size={20}/></button><a className="hero-secondary" href="#sample">Try a question <ArrowDownRight size={18}/></a></div>
    ...
    <section className="section offer-section" id="pricing"><div className="container offer-layout"><div className="offer-copy"><div className="section-kicker">04 / YOUR ACCESS</div><h2>A small price<br/>for <em>better practice.</em></h2><p>One payment for the main paper, explained answers and online subject tests.</p><div className="offer-reassurance"><ShieldCheck size={19}/><span>Access is given only after the payment provider confirms your order.</span></div></div><div className="offer-card"><div className="offer-card-top"><span>THE COMPLETE PRACTICE KIT</span><span>2026 SERIES</span></div><div className="offer-price">₹{price}<small>one-time</small></div><ul><li><Check size={17}/> Main practice paper PDF</li><li><Check size={17}/> Answer explanations</li><li><Check size={17}/> Subject-wise online tests</li><li><Check size={17}/> Return to your student library</li></ul><button className="btn btn-coral" onClick={buy} disabled={!owned && !checkoutReady}>{owned ? 'Open your library' : checkoutReady ? `Get access for ₹${price}` : 'Launching soon'}<ArrowUpRight size={20}/></button><p>{catalog.checkoutMode === 'test' ? 'Local test checkout · no real charge' : catalog.checkoutMode === 'unavailable' ? 'Checkout is opening soon; purchases are not being accepted yet.' : 'Secure checkout · pay once'}</p></div></div></section>
    ...
    <section className="closing-section"><div className="container closing-layout"><span>CIVILPRELIMS / 2026</span><h2>Be ready to answer.<br/><em>Be ready to reason.</em></h2><button className="btn btn-light" onClick={buy} disabled={!owned && !checkoutReady}>{owned ? 'Open your library' : checkoutReady ? `Get the paper for ₹${price}` : 'Launching soon'}<ArrowUpRight size={20}/></button></div></section>
    <div className="mobile-purchase-bar"><div><span>{owned ? 'YOUR ACCESS' : checkoutReady ? 'COMPLETE KIT' : 'LAUNCHING SOON'}</span><strong>{owned ? 'Ready to study' : checkoutReady ? <>₹{price} <small>one-time</small></> : 'CivilPrelims'}</strong></div><button onClick={buy} disabled={!owned && !checkoutReady}>{owned ? 'Open library' : checkoutReady ? 'Get paper' : 'Coming soon'} <ArrowUpRight size={18}/></button></div>
  </main>;
}
